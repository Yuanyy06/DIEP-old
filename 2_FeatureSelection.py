...
# @IDE :        PyCharm
# @JDK :        Python 3.7.1
# @Description :由于上面的问题，这里我们不用RFE，而用他的思路。①对200个training set构建模型，用默认参数。②然后求feature importance的均值和oob score的均值。
# ③删除feature importance最小的 ④重新构建模型，重复上述步骤，直到剩一个特征。
...
delf = -1
for f in range(20):
    # the first delf is -1, no feature is deleted
    print(20-f)
    if delf is not -1:
        del features[delf]
    # lists for storing paras
    oob_score = []
    Accuracy = []
    Recall = []
    Precision = []
...
# output
Results = open("FeatureSelectionparas_training.txt", "a")
